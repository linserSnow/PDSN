...
def extractDeepFeature(img, model, is_gray, mask):
    if is_gray:
...
def eval(model, model_path=None, is_gray=False):
    ...
    with torch.no_grad():
        for i in range(6000):
            ...
            masks = torch.ones(512, 7, 6)
            f1 = extractDeepFeature(img1, model, is_gray, masks)
            f2 = extractDeepFeature(img2, model, is_gray, masks)
